...
class TDVP:
    ...
    def right_sweep_single_site(self,delta):
        for i in range(self.L):
            ...
            psi = local_exponentiation_single_site('Heff_single', self.psi.tensors[i], self.L_env[i-1], self.H.tensors[i], self.R_env[i+1], delta)
    ...
    def left_sweep_single_site(self,delta):
        for i in range(self.L-1,-1,-1):
            ...
            if i != 0:
                ...
                psi = local_exponentiation_single_site('Hfree', C, self.L_env[i-1],' ', self.R_env[i], -delta)
    ...
    def right_sweep_two_sites(self,delta):
        for i in range(self.L-1):
            ...
            psi = local_exponentiation_two_sites('Heff_two', M12, self.L_env[i-1], self.H12[i],self.R_env[i+2], delta)
    ...
    def left_sweep_two_sites(self,delta):
            for i in range(self.L-1,0,-1):
                ...
                psi  = local_exponentiation_two_sites('Heff_two', M12, self.L_env[i-2], self.H12[i-1], self.R_env[i+1], delta)
                (U,S,V), err = svd_truncate(psi.reshape(shp1[0]*shp1[1],shp2[1]*shp2[2]),self.options)
...
def local_exponentiation_single_site(method, M, L, H, R, delta):
    ...
    v = lanczos.expm_krylov_lanczos(Afunc, M.ravel(), -1j*delta/2, numiter=10)
    return v/np.linalg.norm(v)

def apply_Heff_two_sites(L,H12,R,M12):
    return oe.contract('ijk,jalmbc,nad,ilmn->kbcd',L,H12,R,M12)
def local_exponentiation_two_sites(method, M, L, H, R, delta):
    if   method == 'Heff_two':
